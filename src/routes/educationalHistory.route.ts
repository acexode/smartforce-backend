...
export class EducationalHistoryRoute implements Routes {
  ...
  private initializeRoutes() {
    this.router.get(`${this.path}`,AuthoriseRole([Roles.Admin,Roles.dataEntry]),  this.educationalHistory.getEducationalHistory);
    this.router.get(`${this.path}/:id(\\d+)`,AuthoriseRole([Roles.Admin,Roles.dataEntry,Roles.Officer]),  this.educationalHistory.getEducationalHistoryById);
    this.router.post(`${this.path}`,AuthoriseRole([Roles.Admin,Roles.dataEntry]),  ValidationMiddleware(CreateEducationalHistoryDto), this.educationalHistory.createEducationalHistory);
    this.router.put(
      `${this.path}/:id(\\d+)`,
    ...
    this.router.delete(`${this.path}/:id(\\d+)`,AuthoriseRole([Roles.Admin]), this.educationalHistory.deleteEducationalHistory);
  }
}
